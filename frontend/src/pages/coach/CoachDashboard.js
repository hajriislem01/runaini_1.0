...
const CoachDashboard = () => {
  ...
  const fetchAll = async () => {
    ...
    try {
      ...
      const [pRes, gRes, sRes, rRes, summaryRes] = await Promise.all([
        API.get('players/'),
        API.get('groups/'),
        API.get('trainings/'),
  ...
  const quickActions = [
    ...
    { title:'Create Training', desc:'Plan a session',      icon:<FaDumbbell style={{ fontSize:22 }}/>, gradient:'from-[#00d0cb] to-[#4fb0ff]', path:'/coach/training/create' },
                  ...
                  return (
                    <div key={p.id||i}
                      ...
                      <div className="flex items-center gap-2">
                        <div className="w-7 h-7 rounded-full bg-gradient-to-r from-[#902bd1] to-[#4fb0ff] flex items-center justify-center text-white text-xs font-bold flex-shrink-0">
