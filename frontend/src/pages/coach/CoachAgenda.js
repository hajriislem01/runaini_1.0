...
const CoachAgenda = () => {
  ...
  const fetchAll = async () => {
    ...
    } catch { toast.error('Failed to load agenda'); }
  ...
  const filtered = useMemo(() => {
    return sessions.filter(s => {
      ...
      const mgCat    = filterCats.length === 0 || filterCats.includes(s.category);
  ...
  const SessionCard = ({ s, compact = false }) => {
    ...
    const indivEx = (s.exercises||[]).filter(ex => ex.assigned_to && ex.assigned_to !== 'all');
        ...
        <div className="p-4">
          ...
          <div className="flex items-start justify-between gap-3 mb-3">
            <div className="flex-1">
              ...
              <div className="flex items-center gap-3 text-xs text-gray-500 flex-wrap">
                <span className="flex items-center gap-1">
                  <FiClock size={11}/>{fmtTime(s.start_time)} – {fmtTime(s.end_time)}
            ...
            <div className="flex flex-col items-end gap-1.5">
              ...
              <span className="text-xs text-gray-600">Level {s.level}</span>
            </div>
          </div>
            ...
            <div className="mb-3">
              ...
              <div className="space-y-1">
                {(s.exercises||[]).slice(0,3).map((ex,i) => {
                  ...
                  const isInd = ex.assigned_to && ex.assigned_to !== 'all';
                  return (
          ...
          {!past && (
            <div className="flex gap-2 pt-2 border-t border-gray-800">
              ...
              {recur ? (
                <>
                  <button onClick={() => openDelete(s, false)}
                  ...
                  <button onClick={() => openDelete(s, true)}
                    className="flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-xs bg-red-500/10 text-red-400 border border-red-500/20 hover:bg-red-500/20 transition-all">
          ...
          <div className="flex items-center justify-between flex-wrap gap-4">
            ...
            <div className="flex items-center gap-3">
              ...
              <motion.button whileHover={{ scale:1.05 }} whileTap={{ scale:0.95 }}
                onClick={() => navigate('/coach/training/create')}
          ...
          <motion.div variants={iV} className="bg-gray-900/70 rounded-2xl p-6 border border-gray-700/50 mb-8">
            ...
            <div className="space-y-3">
              {upcoming.map(s => {
                ...
                return (
                  <div key={s.id}
                    className="flex items-center gap-3 p-3 rounded-xl border transition-all cursor-pointer hover:border-opacity-60"
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      <div className="text-xs text-gray-500 flex gap-3 flex-wrap">
                        <span>{isT ? 'Today' : format(parseISO(s.date),'EEE MMM d')} · {fmtTime(s.start_time)} – {fmtTime(s.end_time)}</span>
                ...
                return (
                  <motion.div key={i} whileHover={{ scale:1.02 }}
                    ...
                    style={{
                      ...
                      borderColor: todayDay ? '#00d0cb' : 'rgba(30,41,59,.8)',
