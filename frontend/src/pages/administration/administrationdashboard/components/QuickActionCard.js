...
const QuickActionCard = ({ action, navigate, itemVariants, t }) => {
  ...
  return (
    <motion.div variants={itemVariants}
      ...
      <h3 className="text-lg font-semibold text-white mb-1 md:mb-2 break-words">{action.title}</h3>
      <p className="text-gray-400 text-sm mb-3 md:mb-4 break-words">{action.description}</p>
      <div className="flex items-center text-gray-400 font-medium group-hover:text-white transition-colors">
        ...
        <FiArrowRight className="ml-2 rtl:ml-0 rtl:mr-2 group-hover:translate-x-1 rtl:group-hover:-translate-x-1 transition-transform rtl:rotate-180" />
      </div>
    </motion.div>
  );
};
