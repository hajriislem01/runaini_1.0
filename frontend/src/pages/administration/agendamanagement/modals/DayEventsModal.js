...
import {
  FiX, FiCalendar, FiClock, FiMapPin, FiUsers,
  FiEdit, FiTrash2, FiPlus, FiUser,
} from 'react-icons/fi';
import { FaFutbol, FaTrophy, FaDumbbell } from 'react-icons/fa';
import { format } from 'date-fns';

const CATEGORY_COLORS = {
  technical:  { bg: 'rgba(79,176,255,0.15)',  border: 'rgba(79,176,255,0.3)',  text: '#4fb0ff' },
  tactical:   { bg: 'rgba(245,158,11,0.15)', border: 'rgba(245,158,11,0.3)', text: '#f59e0b' },
  physical:   { bg: 'rgba(34,197,94,0.15)',  border: 'rgba(34,197,94,0.3)',  text: '#22c55e' },
  mental:     { bg: 'rgba(168,85,247,0.15)', border: 'rgba(168,85,247,0.3)', text: '#a855f7' },
  match_prep: { bg: 'rgba(239,68,68,0.15)',  border: 'rgba(239,68,68,0.3)',  text: '#ef4444' },
  recovery:   { bg: 'rgba(20,184,166,0.15)', border: 'rgba(20,184,166,0.3)', text: '#14b8a6' },
};
...
const DayEventsModal = ({
  ...
  /* Localised day heading */
  const dayHeading = new Intl.DateTimeFormat(
    lang === 'ar' ? 'ar-EG' : lang === 'fr' ? 'fr-FR' : 'en-US',
    { weekday: 'long', year: 'numeric', month: 'long', day: 'numeric', numberingSystem: 'latn' }
  ).format(selectedDay);

  const renderTypeBadge = (event) => {
    let config = {
      color: 'from-blue-500 to-indigo-600',
      icon: <FiClock />,
      label: t('badges.internalMeeting'),
    };
    if (event.type === 'Match Friendly') {
    ...
    return (
      <span className={`inline-flex items-center gap-1.5 px-3 py-1.5 rounded-full text-[10px] font-bold uppercase tracking-wider text-white bg-gradient-to-r shadow-sm ${config.color} ${isRtl ? 'flex-row-reverse' : ''}`}>
            ...
            <div className={`flex justify-between items-center mb-6 ${isRtl ? 'flex-row-reverse' : ''}`}>
              <div className={`flex items-center gap-3 ${isRtl ? 'flex-row-reverse' : ''}`}>
                ...
                <div className={isRtl ? 'text-right' : ''}>
                  <h2 className="text-xl font-bold text-white capitalize">{dayHeading}</h2>
                  <p className="text-xs text-gray-400 mt-0.5">
                    {t('dayModal.eventsScheduled', { count: dayEvents.length })}
                  </p>
                </div>
              </div>
              <button
              ...
              {dayEvents.map(event => {
                ...
                if (event._isTraining) {
                  ...
                  return (
                    ...
                    >
                      <div className={`flex items-center justify-between px-5 pt-4 pb-3 border-b border-gray-700/50 ${isRtl ? 'flex-row-reverse' : ''}`}>
                ...
                return (
                  <motion.div
                    ...
                    animate={{ opacity: 1, y: 0 }}
                    onClick={() => handleOpenDetail(event)}
                    className={`bg-gray-800/50 rounded-2xl overflow-hidden border border-gray-700/50 p-6 border-l-4 cursor-pointer hover:bg-gray-800 transition-all ${
                      event.type === 'Meeting'       ? 'border-l-blue-500'
                      : event.type === 'Match Friendly' ? 'border-l-orange-500'
                      : event.type === 'Tournament'  ? 'border-l-[#902bd1]'
                      : 'border-l-[#00d0cb]'
                    }`}
                  >
                    <div className={`flex justify-between items-start ${isRtl ? 'flex-row-reverse' : ''}`}>
                      <div className={isRtl ? 'text-right' : ''}>
                        ...
                        <div className={`flex flex-wrap gap-4 text-sm text-gray-300 ${isRtl ? 'flex-row-reverse' : ''}`}>
                          <div className={`flex items-center gap-2 ${isRtl ? 'flex-row-reverse' : ''}`}>
                            ...
                            <span>
                              {event._isTraining
                                ...
                                : (event.groups_detail?.length > 0
                                    ...
                                    : (event.target_coaches?.length > 0 || event.target_players?.length > 0
                      ...
                      {userType !== 'player' && (
                        <div className={`flex gap-2 ${isRtl ? 'flex-row-reverse' : ''}`}>
                          <button
                            onClick={(e) => { e.stopPropagation(); handleEditEvent(event); setShowDayEventsModal(false); }}
                            className="text-[#00d0cb] hover:text-[#4fb0ff] p-2 rounded-lg hover:bg-[#00d0cb]/10"
                          >
                            <FiEdit size={20} />
                          </button>
                          <button
                            onClick={(e) => { e.stopPropagation(); setEventToDelete(event); setShowDeleteConfirm(true); setShowDayEventsModal(false); }}
