...
import {
  FiMail, FiPhone, FiTarget, FiLink, FiSettings, FiEye, FiEdit2
} from 'react-icons/fi';

import { useAdminData } from '../../../context/AdminContext';
import { getCountryName } from './utils/profileHelpers';
import { TabButton } from './components/ProfileUI';
...
const Profile = () => {
  ...
  const renderContent = () => {
    switch (activeTab) {
      case 'overview':
        return <OverviewSection academy={academy} />;
      case 'social':
        return <SocialSection academy={academy} />;
      default:
        return null;
    }
  };
    ...
    <div className="min-h-screen bg-black text-white p-4 sm:p-6 md:p-8">
      <div className="max-w-7xl mx-auto">
        ...
        <div className="flex flex-wrap gap-2 md:gap-3 mb-6 md:mb-8">
          <TabButton name="Overview" icon={<FiTarget size={18} />} isActive={activeTab === 'overview'} setActiveTab={setActiveTab} />
          <TabButton name="Social" icon={<FiLink size={18} />} isActive={activeTab === 'social'} setActiveTab={setActiveTab} />
          <TabButton name="Settings" icon={<FiSettings size={18} />} isActive={activeTab === 'settings'} setActiveTab={setActiveTab} />
        </div>
