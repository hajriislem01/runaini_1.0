...
const PlayerTraining = () => {
  ...
  const filtered = useMemo(() => {
    ...
    return myGroupSessions.filter(s => s.category === filterCat);
    ...
    return (
      <motion.div initial={{ opacity:0, y:6 }} animate={{ opacity:1, y:0 }}
        ...
        style={{
          ...
          borderColor: expanded ? cat.color : past ? 'rgba(30,41,59,.6)' : 'rgba(51,65,85,.4)',
          opacity: past ? 0.72 : 1,
        }}>
        {/* Card header */}
            ...
            <div className="flex-1 min-w-0">
              ...
              <div className="flex items-center gap-3 text-xs text-gray-500 flex-wrap">
                ...
                {s.start_time && s.end_time && (
                  <span className="flex items-center gap-1">
                    <FiClock size={10}/>{fmtTime(s.start_time)} – {fmtTime(s.end_time)}
                ...
                <span style={{ color:cat.color }}>Level {s.level}</span>
              </div>
            </div>
                            ...
                            return (
                              <div key={s.id}
                                ...
                                onClick={() => {
                                  setView('list');
                                  setListTab('upcoming');
                                  setTimeout(() => setExpandedId(s.id), 100);
                                }}>
                    ...
                    return (
                      <motion.div key={i} whileHover={{ scale:1.04 }}
                        ...
                        onClick={() => setSelectedDay(
                          selectedDay && isSameDay(day.date, selectedDay) ? null : day.date
                        )}>
                  ...
                  {selectedDay && (
                    <motion.div initial={{ opacity:0, y:8 }} animate={{ opacity:1, y:0 }} exit={{ opacity:0 }}>
                      <div className="flex items-center gap-2 mb-3">
                        ...
                        <button onClick={() => setSelectedDay(null)} className="text-gray-600 hover:text-white ml-auto">
                          <FiX size={14}/>
                        </button>
                      </div>
                      {(() => {
                        const ds = filtered.filter(s => isSameDay(parseISO(s.date), selectedDay));
                        return ds.length > 0 ? (
                          <div className="space-y-3">
                            {ds.map(s => <SessionCard key={s.id} s={s}/>)}
                          </div>
                        ) : (
                          <div className="text-center py-8 bg-gray-900/40 rounded-xl border border-gray-700/50">
