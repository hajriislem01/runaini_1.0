...
import {
  FiEdit2, FiPhone, FiMapPin, FiActivity, FiCalendar,
...
const PlayerProfile = () => {
  ...
  const { player, isLoading: playerLoading, playerName, playerInitial, photoUrl } = usePlayer();
  const { t, i18n } = useTranslation('playerprofile');
  ...
  if (!player) return (
    <div className="min-h-screen flex flex-col items-center justify-center p-6"
      ...
      <h2 className="text-xl text-gray-300 mb-2">{t('common.noProfileFound')}</h2>
  ...
  const positionKey = player.position?.toLowerCase();
  const positionLabel = positionKey
    ? t(`positions.${positionKey}`, { defaultValue: player.position })
    : '—';

  // Localized status label
  const statusKey = player.status?.toLowerCase();
      ...
      <div className="max-w-7xl mx-auto">
        ...
        <motion.div
          ...
          style={{
            ...
            borderRadius: 'var(--dashboard-radius, 1.5rem)',
          }}
        >
          {/* Decorative blobs */}
          <div className={`absolute top-0 ${isRtl ? 'left-0' : 'right-0'} w-64 h-64 bg-white/5 rounded-full -m-16`} />
          <div className={`absolute bottom-0 ${isRtl ? 'right-0' : 'left-0'} w-48 h-48 bg-white/5 rounded-full -m-24`} />
          ...
          <div className="relative z-10 flex flex-col md:flex-row items-center gap-6 md:gap-8">
            ...
            <div className={`flex-1 text-center ${isRtl ? 'md:text-right' : 'md:text-left'} w-full`}>
              <div className={`flex flex-col md:flex-row md:items-center justify-between gap-4 mb-4`}>
                <div>
                  <div className={`flex items-center justify-center ${isRtl ? 'md:justify-end' : 'md:justify-start'} gap-3 flex-wrap`}>
                    <h1 className="text-2xl md:text-3xl font-bold text-white">{playerName}</h1>
                  ...
                  {player.academy?.name && (
                    <p className="mt-1 md:mt-2 text-gray-300 text-sm">
                      {player.academy.name}
                    </p>
                  )}
                </div>
                ...
                <div className="flex gap-2 self-center">
                  <motion.button whileHover={{ scale: 1.05 }} whileTap={{ scale: 0.95 }}
                    ...
                    className="px-4 py-2.5 text-white shadow-lg flex items-center justify-center gap-2 transition-all rounded-xl"
                    style={{ background: 'var(--main-gradient)' }}>
                    <FiEdit2 />{t('actions.editProfile')}
                  </motion.button>
                </div>
              </div>

              {/* Quick bio badges */}
              <div className={`flex flex-wrap justify-center ${isRtl ? 'md:justify-end' : 'md:justify-start'} gap-3 mt-4`}>
                {player.position && (
                  <div className="flex items-center gap-2 bg-white/10 backdrop-blur-sm px-3 py-1.5 rounded-lg text-white border border-white/20">
                    <FaFutbol size={14} />
                    <span className="text-sm font-semibold">{positionLabel}</span>
                  </div>
                )}
                {player.group?.name && (
                  <div className="flex items-center gap-2 bg-white/10 backdrop-blur-sm px-3 py-1.5 rounded-lg text-white border border-white/20">
        ...
        <div>
              ...
              {[
                {
                  ...
                  value: `${parseFloat(report.overall_score).toFixed(1)}/10`,
                  color: scoreColor(report.overall_score),
                  icon: <FiActivity size={16} />,
                },
                {
                  label: t('stats.attendance'),
                  value: report.attendance_total > 0
                ...
                },
                {
                  ...
                  value: report.is_injured ? t('status.injured') : t('status.good'),
                  color: report.is_injured ? '#f87171' : '#4ade80',
                  icon: <FaHeartbeat style={{ fontSize: 16 }} />,
                },
                {
                  label: t('stats.schoolGrade'),
                  value: report.school_grade_avg
                    ? `${parseFloat(report.school_grade_avg).toFixed(1)}/20`
                    : '—',
                  color: '#a855f7',
                  icon: <FaGraduationCap style={{ fontSize: 16 }} />,
                },
              ].map((s, i) => (
                <div key={i} className="bg-gray-900/70 p-5 flex items-center gap-3 border border-gray-700/50 rounded-2xl">
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
            ...
            <motion.div variants={iV} className="space-y-5">
              ...
              {/* Contact */}
              <div className="bg-gray-900/70 p-5 border border-gray-700/50 rounded-2xl">
              ...
              <div className="bg-gray-900/70 p-5 border border-gray-700/50 rounded-2xl">
                ...
                <div className="space-y-2">
                  {[
                    ...
                    { label: t('labels.subgroup'), value: player.subgroup?.name || '—', color: '#a855f7' },
                  ].map((row, i) => (
              ...
              <div className="bg-gray-900/70 p-5 border border-gray-700/50 rounded-2xl">
                <h2 className="text-sm font-bold text-white mb-4 flex items-center gap-2">
                  <div className="w-1 h-4 rounded-full bg-[#00d0cb]" />
                  {t('sections.quickAccess')}
                </h2>
                <div className="space-y-2">
                  {[
                    { label: t('quickAccess.myTraining'), path: '/players/training', color: '#4fb0ff' },
                    { label: t('quickAccess.performance'), path: '/players/performance', color: '#00d0cb' },
                    { label: t('quickAccess.settings'), path: '/players/settings', color: '#f59e0b' },
                  ].map((l, i) => (
                    <button key={i} onClick={() => navigate(l.path)}
                      ...
                      <FiChevronRight size={13} className={`text-gray-600 ${isRtl ? 'rotate-180' : ''}`} />
                    </button>
                  ))}
                </div>
              </div>
            ...
            <div className="lg:col-span-2 space-y-5">
              ...
              {/* Bio / About */}
              <motion.div variants={iV} className="bg-gray-900/70 p-5 border border-gray-700/50 rounded-2xl">
                <h2 className="text-sm font-bold text-white mb-4 flex items-center gap-2">
                ...
                {player.notes ? (
                  ...
                  <p className="text-gray-600 text-sm">{t('about.noBio')}</p>
                )}
              </motion.div>
                ...
                <motion.div variants={iV} className="bg-gray-900/70 p-5 border border-gray-700/50 rounded-2xl">
                  <div className="flex items-center justify-between mb-4">
                    <h2 className="text-sm font-bold text-white flex items-center gap-2">
                      ...
                      {t('evaluation.latestTitle', { month: parseReportMonth(report.month, i18n.language) })}
                    </h2>
                  ...
                  <div className="space-y-2.5 mb-4">
                    {[
                      ...
                      <div key={i} className="flex items-center gap-3">
                        ...
                        <div className={`text-xs font-bold w-8 ${isRtl ? 'text-left' : 'text-right'}`} style={{ color: p.color }}>
                  ...
                  {(report.comment || report.strength || report.to_improve) && (
                    <div className="p-3 rounded-xl border"
                      ...
                      <div className="flex gap-3 flex-wrap text-xs">
                        {report.strength && (
                          <span className="text-green-400">✓ {report.strength}</span>
                        )}
                        {report.to_improve && (
                          <span className="text-amber-400">→ {report.to_improve}</span>
                        )}
                      </div>
                    </div>
                  )}
              ...
              {!loadingExtra && !report && (
                <motion.div variants={iV}
                  className="bg-gray-900/70 p-10 border border-dashed border-gray-700/50 text-center rounded-2xl">
                  <FiActivity className="mx-auto text-4xl text-gray-600 mb-3" />
                  <p className="text-gray-400 text-sm">{t('evaluation.noEvaluation')}</p>
              ...
              <motion.div variants={iV} className="bg-gray-900/70 p-5 border border-gray-700/50 rounded-2xl">
                <h2 className="text-sm font-bold text-white mb-3 flex items-center gap-2">
                  <FaHeartbeat style={{ fontSize: 14, color: '#ef4444' }} />
                  {t('sections.healthStatus')}
                </h2>
                  ...
                  <div className="p-3 rounded-xl flex items-center gap-3"
                    ...
                    <div>
                      <div className="text-xs font-semibold"
                        ...
                        {player.status === 'Injured' ? t('health.currentlyInjured') : t('health.fitToPlay')}
                      </div>
                      <div className="text-xs text-gray-500">{t('health.playerStatus')}</div>
                    </div>
                  </div>
                    ...
                    <div className="p-3 rounded-xl flex items-center gap-3"
                      ...
                      <div>
                        <div className="text-xs font-semibold"
                          ...
                          {report.medical_cert_valid ? t('health.valid') : t('health.invalidExpired')}
                        </div>
                        <div className="text-xs text-gray-500">{t('health.medicalCertificate')}</div>
                      </div>
                    </div>
                  )}
                </div>
