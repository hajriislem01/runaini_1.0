...
/**
 * Props:
 *   label    – translated string from t()
 *   icon     – react-icon component
 *   error    – error string or undefined
 *   rightEl  – optional JSX for right-side adornment (e.g. show/hide toggle)
 *   isRtl    – boolean – flips label and icon sides for RTL languages
 *   ...props – all native <input> props (type, value, onChange, etc.)
 */
const GoldInput = ({ label, icon: Icon, error, rightEl, isRtl, ...props }) => {
  const [focused, setFocused] = useState(false);
  const hasValue = String(props.value || '').length > 0;

  return (
    <div className="relative">
      {/* Floating label – mirrors to right side in RTL */}
      <label
      ...
      {/* Icon – mirrors to right side in RTL */}
      <div
        className={`absolute ${isRtl ? 'right-3.5' : 'left-3.5'} top-1/2 -translate-y-1/2 z-10`}
        style={{ color: focused ? G : error ? '#f87171' : '#4b5563' }}>
        <Icon size={16} />
      </div>

      {/* The actual input – padding mirrors for RTL */}
      <input
        {...props}
      ...
      {/* Right adornment (show/hide toggle) – mirrors to left in RTL */}
      {rightEl && (
        <div className={`absolute ${isRtl ? 'left-3.5' : 'right-3.5'} top-1/2 -translate-y-1/2 z-10`}>
          {rightEl}
        </div>
      )}
    </div>
...
const Login = () => {
  ...
  useEffect(() => {
    ...
    if (token && user) {
      try {
        ...
        if (u.role === 'superadmin') navigate('/super-admin-portal', { replace: true });
        else if (u.role === 'admin')  navigate('/administration/dashboard', { replace: true });
        else if (u.role === 'coach')  navigate('/coach/dashboard', { replace: true });
        else if (u.role === 'player') navigate('/players', { replace: true });
      } catch {}
    }
  }, [navigate]);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (!userData) {
        setError(t('loginMissingData', 'Login successful but user data is missing.'));
        return;
      }
      ...
      setTimeout(async () => {
        if (userData.role === 'superadmin') {
          navigate('/super-admin-portal', { replace: true });
  ...
  return (
    <div
      ...
      <Link
        to="/"
        className={`absolute top-6 ${isRtl ? 'right-6' : 'left-6'} z-20 flex items-center gap-1.5 text-sm text-gray-500 hover:text-white transition-colors group`}>
        ...
        <div
          ...
          <div className="px-8 py-10">
            ...
            <div className="text-center mb-8">
              <div className="inline-flex items-center gap-1 text-3xl font-extrabold mb-3" dir="ltr" style={{ unicodeBidi: 'isolate' }}>
            ...
            <div className={`mb-6 ${isRtl ? 'text-right' : 'text-left'}`}>
              <h1 className="text-xl font-extrabold text-white">{t('welcomeBack')}</h1>
              <p className="text-sm text-gray-500 mt-1">
                {t('noAccount')}{' '}
                <Link to="/request-academy" className="font-semibold transition-colors" style={{ color: G }}>
                  {t('requestAccess')} {isRtl ? '←' : '→'}
                </Link>
              </p>
            </div>
              ...
              <div className={`flex items-center justify-between pt-1 ${isRtl ? 'flex-row-reverse' : ''}`}>
                <label className="flex items-center gap-2 cursor-pointer select-none">
                  <div
                    ...
                    style={{
                      background: remember ? G : 'rgba(255,255,255,0.06)',
                      border: `1px solid ${remember ? G : 'rgba(255,255,255,0.15)'}`,
                    }}>
                    {remember && <FiCheckCircle size={10} className="text-black" />}
                  </div>
                  <span className="text-xs text-gray-500">{t('rememberMe')}</span>
                </label>
                <Link to="/forgot-password" className="text-xs transition-all hover:text-white"
                  style={{
                    color: '#c084fc',
                    textShadow: '0 0 10px rgba(144, 43, 209, 0.5)',
                  }}>
                  {t('forgotPassword')}
                </Link>
              </div>
            ...
            <div className="flex items-center gap-3 my-6">
              ...
              <span className="text-xs text-gray-600">{t('orContinue')}</span>
              <div className="flex-1 h-px" style={{ background: 'rgba(255,255,255,0.06)' }} />
            </div>
