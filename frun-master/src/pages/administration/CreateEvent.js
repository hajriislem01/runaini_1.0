...
const CreateEvent = () => {
  ...
  // ✅ Fetch subgroups quand le groupe change
  useEffect(() => {
    ...
    fetchSubgroups();
    // Reset subgroup quand groupe change
    ...
    >
      ...
      <motion.div variants={itemVariants}
        ...
        <div className="max-w-7xl mx-auto">
          ...
          <p className="text-gray-300 mt-2">Choose the type of event you want to create • All fields are required</p>
        </div>
      </motion.div>
      ...
      <div className="max-w-7xl mx-auto px-4 py-8 sm:px-6 lg:px-8">
        ...
        <motion.div variants={itemVariants} className="flex flex-col md:flex-row gap-6 mb-8">
          {[
            { tab: 'matchFriendly', icon: <FaFutbol className="text-2xl" />, title: 'Match Friendly', desc: 'Schedule friendly matches with other academies', activeColor: 'border-[#00d0cb] bg-gradient-to-br from-[#4fb0ff]/20 to-[#00d0cb]/20', iconColor: 'from-[#00d0cb] to-[#4fb0ff]', chevronColor: 'text-[#00d0cb]' },
            { tab: 'tournament', icon: <FaTrophy className="text-2xl" />, title: 'Tournament', desc: 'Organize competitive tournaments with multiple teams', activeColor: 'border-[#902bd1] bg-gradient-to-br from-[#902bd1]/20 to-[#4fb0ff]/20', iconColor: 'from-[#902bd1] to-[#00d0cb]', chevronColor: 'text-[#902bd1]' }
          ].map(item => (
            <motion.button key={item.tab} whileHover={{ scale: 1.02 }} whileTap={{ scale: 0.98 }}
              className={`flex-1 p-6 rounded-2xl backdrop-blur-sm border-2 transition-all duration-300 ${activeTab === item.tab ? item.activeColor : 'border-gray-700/50 bg-gray-900/30 hover:border-gray-600'}`}
              onClick={() => setActiveTab(item.tab)}>
              <div className="flex items-center gap-4">
                <div className={`p-4 rounded-xl bg-gradient-to-br ${activeTab === item.tab ? item.iconColor : 'bg-gray-800'}`}>
                  {item.icon}
                </div>
                ...
                {activeTab === item.tab && <FiChevronRight className={`ml-auto ${item.chevronColor} text-2xl`} />}
              </div>
            </motion.button>
          ))}
        </motion.div>
        ...
        <motion.div variants={itemVariants}>
          <div className="bg-gray-900/50 backdrop-blur-sm rounded-2xl border border-gray-700/50 p-6 md:p-8">
            <div className="mb-6 flex items-center gap-3">
              <div className={`p-3 rounded-xl bg-gradient-to-br ${activeTab === 'matchFriendly' ? 'from-[#00d0cb] to-[#4fb0ff]' : 'from-[#902bd1] to-[#00d0cb]'}`}>
              ...
              <div>
                ...
                <p className="text-gray-400">Fill in all required information below</p>
              </div>
            </div>
            ...
            <form onSubmit={handleSubmit} className="space-y-6">
              ...
              <div>
                ...
                <input type="text" name="eventName" value={formData.eventName}
                  ...
                  className={`w-full px-4 py-3 bg-gray-800/50 border ${errors.eventName ? 'border-red-500' : 'border-gray-700'} rounded-xl focus:outline-none focus:ring-2 focus:ring-[#00d0cb] text-white placeholder-gray-500`} />
                {errors.eventName && <p className="text-red-400 text-sm mt-2">• {errors.eventName}</p>}
              </div>

              {/* ✅ Group & Subgroup depuis l'API */}
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div>
                  ...
                  {errors.group && <p className="text-red-400 text-sm mt-2">• {errors.group}</p>}
                </div>
                ...
                <div>
                  ...
                  <div className="relative">
                    <select name="subgroup" value={formData.subgroup} onChange={handleChange}
                      disabled={!formData.group || subgroups.length === 0}
                      ...
                      <option value="" className="bg-gray-900">
                        {!formData.group ? 'Select group first' : subgroups.length === 0 ? 'No subgroups' : 'Select subgroup'}
                      </option>
                      {subgroups.map(sub => (
              ...
              {activeTab === 'matchFriendly' ? (
                <div>
                  ...
                  <input type="text" name="targetAcademy" value={formData.targetAcademy}
                    onChange={handleChange} placeholder="Enter target academy name"
                ...
                <div>
                  ...
                  <input type="number" name="numParticipants" value={formData.numParticipants}
                    onChange={handleChange} min="2" placeholder="Enter number of participants"
                  ...
                  {errors.numParticipants && <p className="text-red-400 text-sm mt-2">• {errors.numParticipants}</p>}
                </div>
              )}
              ...
              <div>
                ...
                <input type="text" name="location" value={formData.location}
                  onChange={handleChange} placeholder="Enter event location"
              ...
              <div className="pt-6">
                <div className="flex flex-col sm:flex-row gap-4">
                  ...
                  <motion.button type="submit" whileHover={{ scale: 1.02 }} whileTap={{ scale: 0.98 }}
                    ...
                    className="flex-1 px-8 py-3 text-white font-semibold rounded-xl transition-all disabled:opacity-70 disabled:cursor-not-allowed flex items-center justify-center gap-2"
                    style={{ background: `linear-gradient(135deg, ${activeTab === 'matchFriendly' ? '#4fb0ff' : '#902bd1'}, ${activeTab === 'matchFriendly' ? '#00d0cb' : '#4fb0ff'})` }}>
                    {isSubmitting ? (
        ...
        <motion.div variants={itemVariants} className="mt-8">
          <div className="bg-gradient-to-r from-[#4fb0ff]/20 to-[#00d0cb]/20 backdrop-blur-sm rounded-2xl border border-[#4fb0ff]/30 p-6">
            ...
            <ul className="space-y-2 text-gray-300">
              <li className="flex items-center gap-2"><span className="text-[#22c55e]">✓</span>All events will be automatically added to the team calendar</li>
              <li className="flex items-center gap-2"><span className="text-[#22c55e]">✓</span>Players will receive notifications 24 hours before the event</li>
              <li className="flex items-center gap-2">
                <span className="text-[#22c55e]">✓</span>
                {activeTab === 'tournament' ? 'Tournament brackets will be generated automatically' : 'Match analytics will be available after completion'}
              </li>
            </ul>
          </div>
        </motion.div>
